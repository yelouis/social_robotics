...
| **Gemma 4** `gemma4:latest` (via Ollama) | Multi-step action expectation & reasoning logic (Layer 03b) | ~9.6 GB | "Extreme SSD" (`OLLAMA_MODELS` external directory) | Apache-2.0 |
...
> [!IMPORTANT]
> **Gemma 4 variant**: Layer 03b's reasoner runs on `gemma4:latest` (the installed ~8B build, ~9.6 GB resident) on `medium`/`large` hosts, and `gemma4:e4b` (4B, 4-bit, ~2.5 GB) on the `small` tier. A `gemma4:26b` (27B-class) tag was originally registered for the 64 GB Mac Studio, but it was **never pulled locally** — only `gemma4:latest` is installed — so the pipeline runs on `gemma4:latest`, which is adequate for the constrained JSON emotion-reasoning task; re-pull a 26B-class tag here if higher fidelity is wanted. See Resolved Issue #9 in `03b_reasonable_emotion_layer.md` for the wrong-tag fix (the registered-but-absent `gemma4:26b` was silently dropping 03b to its rule-based fallback) and the enforced-timeout routing through `shared.vlm_client.ollama_chat`.
