...
18. **`unload()` Auto-Call Premature on 64 GB Mac Studio Host (Resolved - May 13)**:
    - **Problem**: Resolved Issue #16 added `AttentionLayerPipeline.unload()` and a `__enter__/__exit__` protocol so the E2E orchestrator could free L2CS-Net's ~200 MB ResNet50 graph (plus ~500 MB of cached MPS activations) before transitioning to Layers 03b–03g. That unconditional unload was correct for the 24 GB Mac mini M4 Pro, which could not keep L2CS resident alongside Py-Feat (03b), emotion2vec+ (03c), and Depth Anything V2 + SAM (03d). On the new Mac Studio (M4 Max, 64 GB unified memory) host the steady-state combined resident set of L2CS plus every downstream 03 model totals ~12 GB — well under the 40 GB budget — so forcing an unload only costs a ~1.5 s reload + MPS warm-up if a downstream layer re-queries 03a's gaze model, with no compensating memory win.
    - **Solution**: Added a `HIGH_MEMORY_HOST_BYTES = 48 * 2**30` class constant and a `host_can_retain_resident()` static method to `AttentionLayerPipeline` in `src/layer_03a_attention/pipeline.py`. The method returns `True` when `psutil.virtual_memory().total >= 48 GB` (falling back to `False` — i.e. unload — when `psutil` is unavailable), giving the E2E orchestrator a one-line gate to skip the post-`run()` `unload()` call and retain the pipeline instance for the whole 03 run on high-memory hosts. The `unload()` method and the `__enter__/__exit__` context manager are intentionally left unchanged, so direct callers using `with AttentionLayerPipeline(...) as p:` still get deterministic cleanup and memory-constrained hosts still unload. Two unit tests in `tests/test_03a_attention.py` (`TestHostMemoryGate`) cover the 64 GB-retains-resident and 24 GB-unloads branches. Note: the repository currently has no standalone E2E orchestrator module, so the gate ships as a tested helper for the orchestrator to consume rather than being wired into a live call site.

19. **L2CS-Net (ResNet50 Backbone) Gaze Model Upgrade Deferred (Resolved - May 13)**:
    - **Problem**: `L2CS-Net` was originally selected (Resolved Issue #1) because its ~200 MB ResNet50 backbone fit the 24 GB Mac mini M4 Pro's MPS-resident budget and the upstream `models/l2cs-net/` repo could be patched in-tree for the `include_detector=False` path. The model dates from 2022, and its ~10° angular error on Gaze360 is now eclipsed by 3DGazeNet and CrossGaze (~7° on Gaze360) — both Apache-2.0, both with ~500 MB-1 GB checkpoints that would not have fit alongside the rest of the 03 layers on a 24 GB host but now fit comfortably on the Mac Studio (M4 Max, 64 GB unified memory) target. The open question was whether the new memory headroom alone justified migrating off a 4-year-old gaze backbone.
    - **Solution**: Decision recorded to **hold on L2CS-Net** rather than pursue a speculative upgrade; no code change was made. The A/B-migration path (adding `models/crossgaze/`, a second `gaze_pipeline` adapter, and a 100-clip ground-truth comparison harness) was assessed as ~1-2 weeks of effort with non-trivial regression risk — CrossGaze's coordinate convention may not match the `gazeto3d` convention that Resolved Issues #2 and #6 calibrated `_track_and_score` against. Since no downstream layer has yet reported a failure attributable to L2CS pitch/yaw noise, the migration carries cost and regression risk with no measured benefit. The upgrade will be revisited only when a concrete downstream failure mode justifies it — specifically, a Layer 03e false-shake / false-nod detection that traces back to noisy L2CS pitch/yaw output. This entry stands as the documented rationale for remaining on L2CS-Net, closing the "no documented reason beyond inertia" gap.
