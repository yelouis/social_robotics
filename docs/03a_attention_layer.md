...
For **each** `person_id` in the manifest's `bystander_detections` array, use the pre-computed bounding boxes as initialization for temporal face/body tracking throughout the clip. If multiple bystanders are present, each is tracked and scored independently. This ensures we capture the attention state of every visible person, not just the most prominent one. **Decoding is single-pass**: at each sampling timestamp the frame is decoded once and every bystander active at that timestamp is cropped, resized to `(448, 448)`, and stacked into one `[N, 448, 448, 3]` L2CS batch — the clip is never re-decoded per bystander (Resolved #1). Because Node-02 persists boxes at only 1/3 FPS, the stale manifest box is also **re-detected per scored frame** (a fresh `yolov8n-pose` max-IoU match, refreshed at most every `SR_03A_REDETECT_INTERVAL` ≈ 0.25 s) so the crop tracks the subject under fast egocentric motion (Resolved #3, #6).
...
- **Window-restricted scoring (throughput)**: only ~19 % of clip-time contains a bystander detection, so gaze scoring runs only within ±`SR_03A_WINDOW_PAD_SEC` (≈ 2 s) windows around bystander-detection timestamps; gap frames are still decoded (decoding stays **sequential and frame-accurate** — a `cap.set`-seek shortcut was tried and **reverted** because it is keyframe-approximate on H.264 and desyncs frame↔timestamp, corrupting scores) but skip the YOLO re-detect + L2CS pass. A clip-level `bystander_face_quality` gate (shared with 03b, but at a looser 03a-tuned tier) skips clips whose faces are too small/blurry for L2CS to resolve. *(Throughput rationale + bit-identical validation: Resolved #5, #6; per-clip `--workers N` parallelism: Resolved #7.)*
...
- **Human-face validity gate (before regression)**: L2CS regresses a confident pitch/yaw for *any* crop — animal, poster, empty background — so each candidate crop is first checked by a MediaPipe BlazeFace face-presence detector (`MIN_FACE_CONF`, default 0.5). Crops with no human face emit `{"score": 0.0, "target": "NoFace"}` and **skip L2CS entirely**, so non-human/degenerate crops can no longer fabricate a gaze score (the original symptom was a confident 0.92 "looking at camera" on a dog). The gate fails **open** (scores the crop) on a model/detector outage so an outage never silently zeros every bystander. *(Resolved #2.)*
...
- `attended_fraction` / `engaged_attention_score`: the **length-invariant** engagement pair — respectively the share of samples scoring ≥ 0.5 (*how often* the bystander looked) and the mean over only score > 0 samples (*how intently* they looked when they did). These are the headline engagement figures: `average_attention_score` dilutes with clip length because every `NoFace`/0.0 sample drags it toward zero (a 4.7 h clip regresses to ~0 regardless of genuine looking), so downstream layers should prefer this pair. *(Resolved #4.)*
...
6. **Corpus-Scale Throughput: Face-Quality Clip Gate + Cadence-Decoupled YOLO (Resolved - June 11)**:
   - **Problem** (was Unresolved Issue 1): After window-restricted sampling (Resolved #5), two costs remained — gap frames were still grabbed (not skipped) for frame-accuracy, and the per-frame YOLO-pose re-detect (Resolved #3) ran on **every** sampled frame within bystander windows, including at the 32 FPS burst cadence where a person's box barely moves between frames. At the 1,000-clip reservoir (median ~26 min/clip) the layer projected to **~18 days**. A June 11 re-assessment found the original deferred-option list stale: the bystander-face-quality pre-pass (03b Resolved #8, shipped after this issue was scoped) gives a clip-level lever that didn't exist on June 3, and the trackers in the original "cheaper re-detect" option carried avoidable drift risk.
   - **Solution** (two compounding levers; the gap-decode floor is split into its own issue below):
     **A′ — clip-level face-quality gate at a 03a-tuned tier.** `run()` now populates the shared `bystander_face_quality` manifest field (idempotent ~0.9 s/clip pre-pass shared with 03b, fail-open on any failure) and skips clips below an env-tunable tier (`SR_03A_FACE_QUALITY_GATE`, `SR_03A_FQ_MIN_PX`/`_CONF`/`_FRAMES`, default **60 px / 0.6 / 2**). The tier is deliberately **looser than 03b's 120 px/0.8/3** — the June 11 join of face quality vs real 03a results proved L2CS gaze resolves smaller faces than HSEmotion emotion: 03b's strict tier would have lost two genuinely-engaged clips (`6fd026d8` eng 0.94, `3f503d0b` eng 0.68), while the 03a tier lost **none** and still skips **54 % of the 50-clip corpus sample**. Clips it skips would emit all-`NoFace`/0.0 traces that 03e rejects anyway, so no downstream information is lost. Instance-toggleable (`enable_face_quality_gate`) like the per-crop face gate.
     **B′ — cadence-decoupled YOLO re-detect.** The gaze-sampling cadence is untouched (03e's 32 FPS medfilt contract requires the dense trace), but `_track_and_score_batched` now refreshes YOLO-pose boxes at most once per `SR_03A_REDETECT_INTERVAL` (default **0.25 s**) and reuses the freshest boxes between refreshes — ≤ 250 ms stale versus the ≤ 2 s-stale manifest boxes that re-detection (Resolved #3) was added to replace, and an **8× cut in YOLO calls during bursts**. The original tracker-based option (KCF/CSRT) was dropped: cached-box reuse achieves the cut with no drift mechanism.
   - **Validation** (June 11, 10-clip sample, production config): **23 min → 9.8 min (2.4×)**; the gate skipped exactly the one zero-signal clip (`10167fcf`, att 0.00) and no real-signal clip; 3/8 kept clips bit-identical, the rest within **≤ 0.07 mean-attention** of v4 (B′'s slightly different boxes shift scores slightly and hence burst triggering — expected, not seek corruption). One honest caveat: borderline clip `067b03df` (mean att 0.07 → 0.01) dropped its `any_person_engaged` flag via the sustained-window path — the weakest clip in the sample, an accepted trade for the corpus-scale gain. Note the 10-clip sample is biased toward strong faces (gate skipped 1/9; the corpus-sample rate is ~54 %), so the corpus-scale gain compounds well beyond 2.4×. Tests: gate tier + fail-open + cadence (13 trace points still emitted, 7 YOLO calls not 13); suite 141/141.

7. **Corpus-Scale Throughput: Gap-Decode — Exact-Seek Rejected, Per-Clip Parallelism Adopted (Resolved - June 13)**:
   - **Problem** (was Unresolved Issue 1): After Resolved #6, decode still ran fully sequentially over the whole clip (`cap.grab()` through the ~81 % bystander-free gap) — the remaining dominant per-clip cost on long reservoir clips. The first-selected fix, a frame-accurate seeking decoder (PyAV), was built and **rejected (June 12) because it failed its own mandatory bit-identical precondition**: a single-frame probe matched cv2 6/6, but the full in-pipeline 10-clip run diverged on **0/8 clips** (trace lengths drifting up to ~2×). Root cause (*not* fps — every clip is exactly 30.0 fps in both decoders): 03a's 32 FPS adaptive burst requests the *same integer frame index* on consecutive samples on a 30 FPS clip, and cv2 (sequential `grab`/`retrieve` with buffer reuse) vs PyAV (a next-frame generator) resolve that repeat differently, cascading through the burst-trigger logic. **Recorded so it is not re-attempted: any decoder whose frame indexing differs from cv2's sequential read — hardware decode included — carries the same risk.** PyAV was reverted and the `av` dependency removed.
   - **Solution** (the bit-identical-safe path): `run(workers=N)` gained an opt-in parallel mode. **`workers=1` (default) is byte-identical to the historical serial path** and the only mode safe for arbitrary library callers. With `workers>1`, whole clips are distributed across a spawn-safe `multiprocessing` Pool (`get_context("spawn")`, module-level picklable `_par_worker_init`/`_par_worker_process`, `cv2.setNumThreads(1)` per worker); each worker builds one `AttentionLayerPipeline` and calls the **unchanged `process_video`**, so every emitted record is identical to serial — only *which process* runs a clip differs, overlapping one clip's CPU decode with another's MPS inference. Results are written incrementally as they complete (resumable; list order is completion order, which the Layer-04 `video_id` outer-join is agnostic to). Opt in via the guarded CLI `PYTHONPATH=src python -m layer_03a_attention.pipeline <manifest> <out> --workers N` (a non-`__main__`-guarded runner must NOT pass `workers>1`, or macOS 'spawn' re-execs it). **Validated bit-identical 3/3** on a real 3-clip sample (serial vs `workers=2`, byte-for-byte per record — confirming MPS inference is deterministic across processes); **~1.4× wall-clock on just 2 workers** (417 s → 294 s), bounded by the single-MPS inference serialization exactly as scoped (the decode share is what overlaps). Orchestration covered by `tests/test_03a_attention.py::TestParallelOrchestration` (result assembly + error recording + incremental write, Pool mocked).
