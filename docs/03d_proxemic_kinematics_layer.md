...
_No actionable issues open — the June 13 smell-test findings (the `person_id` fallback-index collision that produced a confident false Avoidance, and the opaque sentinels on the 30% no-yield corpus) are resolved above (Resolved #4–#5)._
