...
**Status**: ⚠️ Confirmed Unresolved — **dependency now unblocked.** The upstream root cause (**docs/02 Issue 1**) was resolved in the user's favor: **Option A selected** (window-bounded densification pre-pass, 02 untouched) and the pre-pass is implemented (`tools/densify_manifest.py` over `src/shared/dense_detect.py`). Note 03f required **no code change** to benefit — feeding it the dense manifest alone lifted it 0 → 7 (its gate is compatible with dense boxes; docs/03f Resolved #12) — so 03d likewise needs no upstream change and can simply consume the same dense manifest. This issue now awaits **only the 03d selection below**. 03d's proxemic delta needs **≥ 2 endpoints** in the measurement window; at Node-02's 1/3-fps sampling, sparse clips frequently skip as `mixed_skip` / `all_bystanders_single_detection`. A 25-clip A/B (`tools/ab_density.py`) feeding window-dense boxes (`src/shared/dense_detect.py`) moved 03d from **22 → 25** clips with data (recovering `28539222`, `51cb7800`, `599037f1`), **48 → 72** per-person rows, `proxemic_confidence > 0` rows **7 → 11**, and **0 → 2** genuine non-Neutral actions (`66fa8650` Approach_Intervention, `66d4121f` Avoidance). The benefit is **moderate, not strong**: most verdicts stay `Neutral` because a 2 s proxemic delta is physically often neutral and 03d's ≥2-endpoint anchoring already coped in many cases — so unlike 03f (0 → 7 flinches), dense boxes here mainly **recover scorability** rather than unlocking a large hidden signal.
...
Your selection: Proceed with Option A — adopt the window-dense manifest for 03d and add the per-frame proxemic trajectory. Adopting the dense manifest needs no 03d code change (lands with the corpus densification, like 03f); the trajectory is the one 03d code change, to implement as part of the landing.
