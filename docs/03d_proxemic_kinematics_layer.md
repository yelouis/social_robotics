...
### Issue 1: Anchored measurement span is unbounded on sparse tracks (up to 198 s)
**Status**: ⚠️ Confirmed Unresolved — Surfaced by the June 10 post-fix run: `_bystander_measurement_window()` widens to the climax-nearest detection ± 1 *detection index*, so the span in **seconds** is whatever the neighbor-detection gap happens to be. On this corpus **33/112 vectors sat on spans > 30 s (max 198 s)** — a "reaction" delta measured over minutes of track drift is locomotion, not a reaction. In this run the risk stayed latent (**all 33 long-span vectors were zeroed by the ego-motion chaos gate; all 10 confident vectors sit on ≤ 12 s spans**), but a quiet-camera clip with a sparse track would let a long-span vector through at confidence 1.0 with nothing distinguishing it from a tight 6 s measurement.

**Option A (recommended)**: **Hard span cap.** Add `MAX_ANCHOR_SPAN_SEC` (e.g. 30.0) to the tuning-constants block: if the anchored span exceeds it, return `None` (bystander skipped; sentinel semantics unchanged).
  - *Pros*: One constant + one comparison; kills the failure mode outright; provably zero effect on this run's confident vectors (all ≤ 12 s); consistent with the honest-yield philosophy (no data beats wrong data).
  - *Cons*: Skips sparse-track bystanders entirely rather than salvaging them; threshold needs occasional re-tuning against the detection-cadence distribution.

**Option B**: **Confidence attenuation by span.** Keep long spans but scale `proxemic_confidence` down with span length (e.g. linear falloff past 12 s, floor 0), so downstream fusion can weight them.
  - *Pros*: No data discarded; downstream keeps a graded signal.
  - *Cons*: A drift-over-minutes vector is *wrong*, not merely *uncertain* — attenuation still leaks sign information from non-reaction locomotion; adds a second tuning curve.
