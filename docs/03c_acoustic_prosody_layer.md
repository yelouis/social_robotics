...
20. **emotion2vec+ and SenseVoice Identifiers Migrated to Tier-Per-Host Registry (Resolved - May 16)**:
    - **Problem**: Following Resolved Issues #18 (`gemma4:26b` promotion) and #19 (`emotion2vec+ large` retention), every model identifier in 03c was still hardcoded as a string literal at its call site: `AutoModel(model="iic/emotion2vec_plus_large", ...)` in `_init_models`, plus `AutoModel(model="iic/SenseVoiceSmall", ...)` at two call sites (one eager, one lazy). Downgrading the layer to the 24 GB Mac mini fallback variants required editing this file directly, and there was no single place that flipped 03c, 03b, 03d, 03f, and the filtering VLM together for a small-host run.
    - **Solution**: `pipeline.py` now imports `get_model` from `src/models_config.py` and resolves the SER identifier as `get_model("layer_03c_ser")` and the AED identifier as `get_model("layer_03c_aed")` at every call site (eager init, eager-load path on high-memory hosts, lazy-load path in `_run_sensevoice_model`). The `medium`/`large` tiers preserve `iic/emotion2vec_plus_large` + `iic/SenseVoiceSmall`; the `small` tier substitutes `iic/emotion2vec_plus_base` (the Mac mini-compatible SER variant) while keeping `iic/SenseVoiceSmall` unchanged. The `large` tier reserves `iic/emotion2vec_plus_seed` as the future-A/B slot referenced in Resolved Issue #19, so adopting `seed` becomes a one-line edit to `src/models_config.py` instead of touching 03c. The `HIGH_MEMORY_HOST_BYTES` / `host_can_eager_load_sensevoice()` gate from Resolved Issue #18 is unchanged. See `ml_dependencies.md` Resolved Issue #1 for the cross-layer registry rationale.
