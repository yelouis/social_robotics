...
6. **Runner Import Crash + ffmpeg Error-Flood on No-Audio Clips (Resolved - June 25)**:
   - **Problem**: Two issues surfaced on the top-200 run. **(1) Runner crash:** `pipeline.py` imported `Layer03cConfig` with a **bare `from src.layer_03c_acoustic_prosody.config import …`** (no try/except), so it only resolved when run from the project root — under the standard runner (which puts `src/` on `sys.path`, not its parent) it raised `ModuleNotFoundError: No module named 'src'` and the run never started. **(2) ffmpeg error-flood:** 03c attempted ffmpeg extraction on every reaction window, but **~36 % of the corpus's clips have no audio stream** and long clips' audio often ends before a late reaction window, so the seek landed past the audio and ffmpeg failed. Each failure logged at `ERROR` (**1,570** on the first ~40 clips), flooding the log, masking real errors, and wasting a subprocess per no-audio segment. *(Output was already honest — Resolved #1 emits `audio_present=False` for these — so this was a hygiene/throughput bug, not a correctness one.)*
   - **Solution**: **(1)** wrapped the config import in the project's `try: from src.… except ImportError: from …` fallback. **(2)** Added `_audio_duration(video_path)` — a per-clip-cached `ffprobe` of the audio-stream duration (`0.0` = no audio stream; `+inf` = present-but-unknown → let ffmpeg try) — and `_extract_audio_chunk` returns early with a clean `audio_present=False` marker when `start_sec` is at/after it; the residual genuine ffmpeg failure is downgraded to `WARNING`. **Validated on the full top-200**: ffmpeg errors **1,570 → 0**, runtime **696 s**, **200 clips / 0 errors**; **131 clips carry audio → 1,926 audio-present segments, 675 (35 %) non-zero prosody**, tones Soothing 341 / Discouraging 301 / Alarming 33 / Neutral 1,251. Suite **30/30** (`test_audio_chunk_extraction_mock` updated to bypass the new pre-check). A per-clip progress print was also added for run monitorability.

## ⚠️ Unresolved Issues & Suggestions

_None at this time — the June 9 (SER-confidence floor, disgust mapping) and June 25 top-200 run (runner import crash, no-audio ffmpeg flood) findings have been resolved above (Resolved #4–6)._