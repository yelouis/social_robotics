...
- `src/filtering_and_labeling/pipeline.py`:
  ...
  - Gated by the `run_synthetic_qa` toggle (constructor arg, backed by `SAF_RUN_SYNTHETIC_QA`, **default off** as of the June 12 parking decision — Layer 1a is not used for now, see Unresolved Issue 1). The saved synthetic fixtures are referenced from the Extreme SSD and exercised through the filter only when the toggle is explicitly enabled; by default (and when off), `_is_supported_dataset` excludes `synthetic_validation` entries so the run reflects the raw corpus only. Generation itself is a separate one-time step (§4) and is never invoked by the E2E run.
...
**Status**: ⚠️ Confirmed Unresolved — **PARKED (June 12 decision): Layer 1a is not used for now.** Per the operator decision we are not pursuing a render fix at this time; instead the synthetic-QA path is **disabled by default** (`SAF_RUN_SYNTHETIC_QA` now defaults to `0` in `FilteringPipeline`), so normal runs reflect the raw corpus only and never depend on a synthetic fixture, and generation stays a manual opt-in step. This issue is **intentionally retained here (not resolved)** so the render gap and its remediation options (A/B below) stay visible for when synthetic true-positive QA is revisited; re-enable with `SAF_RUN_SYNTHETIC_QA=1`. *(Original failure record:)* June 4 attempts all failed before producing a clip — the 14B model at 81 frames OOM'd in MPS attention (~48 GiB MTLBuffer request), and the 1.3B fallback was SIGKILL'd (exit 137) during CPU-side decode at both 49 and 33 frames. The "generate once, save to SSD, reference thereafter" workflow (§4) is blocked at the "generate once" step.
