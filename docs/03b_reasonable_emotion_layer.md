...
Using the extracted Contextual Task (e.g., "Juggling apples"), prompt a locally running LLM (**Gemma 4** via Ollama) to generate baseline emotional expectations. On the **Mac Studio (M4 Max, 64 GB unified memory)** target host the configured model is the installed `gemma4:latest` (~8B) — `gemma4:26b` was registered but never pulled, so 03b now uses `gemma4:latest` (see Resolved Issue #9; re-pull a 26B-class tag for higher fidelity). Concurrent tracking arrays for multiple bystanders coexist without swapping.
...
We track the bystander within a window anchored to the task. Node-02 ships `task_temporal_metadata = {}` (02 Resolved Issue #8), so 03b first **self-populates the climax / reaction window** via `shared.climax_extraction.populate_climax_for_manifest` (idempotent across layers — a no-op once any Layer 03 has run it; Resolved #5). The strict reaction window is anchored to the *wearer's* optical-flow climax, but the bystander is usually detected slightly off it, so 03b applies a **forward-fixed-width presence anchor** to sample where the bystander actually appears (Resolved #2). A cheap **bystander-face-quality pre-filter** skips clips whose faces are too small/blurry for HSEmotion to resolve *before* paying the climax optical-flow cost (Resolved #8). Each task also expands into one segment per bystander cluster (multi-window — docs/03 § Multi-Window Reaction Segments), so 03b scores a bystander's emotion *trajectory* across the task's segments and applies the cross-layer per-segment guardrails (**skip untracked negative-id bystanders**; **dedupe** segments that collapse to the same presence-anchored window).
1. Run a SOTA emotion model (specifically **HSEmotion-PyTorch**) on the bounding box faces at **3-5 FPS** across the reaction window.
   *Architectural Note:* We explicitly use HSEmotion instead of alternatives like Py-Feat because HSEmotion runs natively on Apple Silicon via Metal Performance Shaders (MPS). CPU-bound emotion models create severe inference bottlenecks on the Mac Studio, so maintaining MPS acceleration is critical for this layer's throughput. The production backend is **`hsemotion-onnx`** (ONNX Runtime): the pipeline prefers ONNX, falls back to the torch build, then to a **loud mock** — never a silent neutral (Resolved #4). Faces are decoded **single-pass and batched** across co-windowed bystanders rather than re-decoding per person (Resolved #6).
   *Reliability gates (honest, not lossy):* (a) a **MediaPipe BlazeFace face-presence gate** rejects non-human / empty / occluded crops before HSEmotion runs (Resolved #3); (b) an **emotion-confidence floor** `MIN_EMOTION_CONF` (default 0.4) drops samples whose top softmax probability is below the floor, so a task with no confident read scores **nothing** rather than feeding near-baseline (~0.18-magnitude) noise into the aggregate. This honesty gate is why 03b is intrinsically **low-yield on distant-bystander egocentric footage** — only the minority of clips with a resolvable, confidently-classified face produce a score (Resolved #7).
...
9. **LLM Reasoner Silently Absent (Wrong Tag) + Un-Timed (Resolved - June 19)**:
   - **Problem**: Two latent issues surfaced in the pre-full-run audit. **(1) Wrong model tag.** `models_config` registered `gemma4:26b` for the 64 GB host, but only `gemma4:latest` (the ~8B build) is installed locally. The availability probe (`ollama_client.list()`) only checks that the ollama *server* is up, not that the *tag* exists — so `self.ollama_available` was `True`, every transition then requested `gemma4:26b`, ollama returned "model not found", and after `LLM_MAX_RETRIES` the code fell through to `_rule_based_classify`. So 03b's scored clips were classified by the **rule-based fallback, never the intended LLM** — a silent *fidelity* regression (it does **not** change yield, which stays face-quality-gated per Resolved Issue #7). **(2) Un-timed calls.** Both `ollama_client.chat(...)` calls had no request timeout; ollama-python ignores its own client timeout, so a wedged generation would hang the unattended batch (the exact failure 02's gate hit — 02 Resolved Issue #20).
   - **Solution**: **(1)** Re-pointed `layer_03b_ollama` (`medium`/`large`) at the installed `gemma4:latest` in `src/models_config.py`; the 8B build is adequate for the constrained JSON emotion-reasoning task (re-pull a 26B-class tag here if higher fidelity is wanted). **(2)** Routed both LLM calls through `shared.vlm_client.ollama_chat` — the enforced-timeout httpx path (`LLM_REQUEST_TIMEOUT=300`, `fmt="json"`): on timeout the call raises (the existing retry → rule-based fallback handles it) and the closed socket cancels the server-side generation. `ollama_client` is retained only for the fast local `.list()` probe. **Validated**: a direct `gemma4:latest` JSON call returns valid JSON (~15 s cold, `{"status":"ok"}`), and a 5-clip 03b run executes the LLM-enabled path end-to-end with **no "model not found" and no hang** (the 5 clips scored 0 — all face-quality-gated, as expected for this low-yield layer, so the per-transition LLM path is covered by the direct call). Suite 200/200.

10. **Multi-Window Guardrails + Long-Clip Anchor Bound (Resolved - June 25)**:
   - **Problem**: 03b expands each task into per-segment pseudo-tasks (multi-window climax, 02 Resolved #22) but had **neither** the cross-layer guardrails **nor** any bound on bystander-anchoring distance. Two failures surfaced on the top-200 run. **(a) Over-count:** a sparse bystander's segments collapse onto the same presence-anchored window, counting the identical reaction once per segment (the 03e over-count, replicated). **(b) Pathological hang:** every bystander was anchored to *its own* nearest detection regardless of distance from the task climax, so on `08a946f4` (a **100-minute** clip with **192 bystanders scattered across 0–5982 s**) the single per-segment decode pass spanned the whole clip — ~180k frame-grabs × 10 segments, hanging the run for hours on one clip. (b) is also wrong to the layer's *motive*: a bystander seen once at t=3000 s is not reacting to a task at t=50 s.
   - **Solution**: **(a)** the two cross-layer per-segment guardrails (docs/03 § Multi-Window Reaction Segments) in `_collect_emotion_timeseries` — an **untracked-track filter** (skip negative `person_id`; untracked 03d Resolved #4 fragments) and a per-clip **distinct-window dedup** (a `scored_windows` set keyed on `(person_id, window)`, threaded from `process_video`). **(b)** `MAX_ANCHOR_SPAN_SEC = 30` (mirrors 03d): a bystander whose nearest detection is more than 30 s from the segment climax is skipped; and the decode now starts at the **first sample frame** rather than frame 0, so a late segment in a long clip no longer grabs every frame from 0. **Validated**: `08a946f4` hung → **99 s**, max per-person count **1**; full top-200 run **88 clips, 0 errors, all `hsemotion_onnx`, dedup median 1× / max 6** (a genuine 6-segment trajectory, not photocopying), `task_aggregate_score` balanced **24 neg / 241 neutral / 24 pos**. Suite 12/12 (`mock_collect` signature updated for the new `scored_windows` arg).
