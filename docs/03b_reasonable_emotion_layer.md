...
16. **`gemma4:e4b` (4B) Default Underutilized 64 GB Mac Studio for Multi-Step Reasoning (Resolved - May 14)**:
    ...
    - **Follow-up (May 16)**: `OLLAMA_MODEL` is now bound to `get_model("layer_03b_ollama")` from the tier-per-host registry (`ml_dependencies.md` Resolved Issue #1). `medium`/`large` tiers resolve to `gemma4:26b` (unchanged Mac Studio default); the `small` tier now re-wires the `gemma4:e4b` fallback for 24 GB hosts via `SR_MODEL_TIER=small`, restoring the pre-Resolved-Issue-#8 escape hatch without touching this file.
