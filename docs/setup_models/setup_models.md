...
- [ ] **Action**: Pull the reasoning-heavy variant for the Appropriateness Judge (Module 05): `ollama pull gemma4:e4b`.
- [ ] **Action**: Pull the high-speed utility variant for Sanity Checks (Module 06): `ollama pull gemma4:e2b`.
- [ ] **Verification**: Run a "Thinking Mode" test: `ollama run gemma4:e4b "<|think|> Test Rs calculation logic."`


### Step 3: Vision Layer (MLX-VLM - Optional/Advanced)
...
> **Note**: This is the original high-performance vision path. Due to library conflicts, the primary implementation uses **Ollama/Moondream** (Step 1). Use MLX-VLM only if Moondream's performance is insufficient.
