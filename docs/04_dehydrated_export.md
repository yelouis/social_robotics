# AI Task Breakdown: Dehydrated Export and Distribution

## Objective
Package the sum total of all the extracted social metadata generated by the Social Feature Layers into a clean format to be posted securely on open data platforms such as Hugging Face.

---

## The "Dehydration" Rule
To strictly adhere to AI ethics, safety frameworks, and copyright logic enforced on varying video datasets (like Ego4D, Epic-Kitchens, etc.), **we do not redistribute source pixels**.

All resulting export files **must be completely dehydrated**. This means:
- No `.mp4`, `.avi`, or `.png` frame exports.
- No inclusion of isolated audio tracks.
- Focus strictly on the extracted parameters: kinematics, layer decisions, VLM contexts, timestamps, and confidence scores.

> **Inverse tool:** Node 05 ([`05_signal_visualizer.md`](05_signal_visualizer.md)) *re*hydrates the same layer outputs **locally** — overlaying them back onto the source pixels for QA — and is deliberately the only place pixels are touched. It is local-only and **never** writes into this export surface; the dehydration rule above is unaffected by it.

---

## Layer-Result Aggregation
Before export, the individual layer output JSONs must be merged into a single master table. Each Social Feature Layer (03a, 03b, etc.) produces its own result file keyed by `video_id`. The aggregation step is responsible for joining these into a unified record per video.

### Aggregation Logic
1. **Scan** the results directory for all `03*_result.json` files.
2. **Outer-join** on `video_id`: Every video that appears in *any* layer's output gets a row. If a layer did not process a given video (or failed), its columns are filled with `null`/`NaN`. Missing `video_id` records are skipped safely.
3. **Flatten nested structures & Extract Summaries**: 
   - **Schema A** (`aggregate` / `per_person`) and **Schema B** (`tasks_analyzed`) are flattened. With the multi-window reaction segments (docs/03 § Multi-Window Reaction Segments), Schema-B `tasks_analyzed` now carries one entry per `(task, segment)` — i.e. multiple rows per `(video_id, person_id)` that form a reaction *trajectory* through the task. The per-layer summary aggregations (mean / max / any) reduce across those rows, and the layers' distinct-window dedup + genuine-track filter ensure the rows are genuine moments rather than the same window counted N times.
   - A centralized `LAYER_SUMMARY_REGISTRY` maps each layer to specific scalar fields (e.g., `avg_task_score`, `max_proxemic_confidence`). These are extracted, aggregated (mean, max, max_abs, any), and surfaced as dedicated Parquet columns. **Classification queries (`any_eq:<label>`) must use the exact label literal the emitting layer outputs** — e.g. 03d emits `Approach_Intervention`/`Avoidance` (not `Approach`/`Retreat`), and 03e its own pinned gesture vocabulary. A mismatch is silently dangerous here: the query simply never matches, producing an all-`None` column instead of an error. To prevent that, `tests/test_layer_04.py::TestRegistryEnumSync` asserts every `any_eq:` literal in the registry is in the corresponding layer's pinned classification vocabulary, so enum drift fails the suite rather than silently deadening an export column (Resolved #1).
   - The full nested arrays are preserved in JSON-encoded string columns (`*_raw`) for research use.
   - **Published header rename**: internally every layer column is namespaced `<layer_id>_<layer_name>_<suffix>` (e.g. `03f_motor_resonance_max_ego_chaos_score`), but the released Parquet strips the internal `03x_` layer-id token (→ `motor_resonance_max_ego_chaos_score`) so consumers don't inherit the repo's private layer-numbering. Only the *number* is removed — the descriptive layer name stays, because dropping it would collide shared metric names (`tasks_analyzed_raw`, `skipped_reason`) across layers (a collision raises rather than silently dropping a column). The `schema_version` hash is recomputed on the published names so it still matches the Parquet.
4. **Top-Level Provenance Flattening**: Top-level dictionaries (such as `processing_meta`) are flattened by one level so that critical provenance details are exposed as Parquet columns (e.g., `<layer>_<dict_key>_<sub_key>`).
5. **Attach manifest metadata**: Join in the `task_labels` (parsed from the `identified_tasks` array), `source_dataset`, `duration_sec`, and `fps` from `filtered_manifest.json` so the exported dataset is self-describing.
6. **Embed export metadata**: 
   - We use a dual-export approach. The dataset is exported as `social_metadata.parquet`, and provenance data is serialized into a companion `export_metadata.json` to avoid issues with Parquet metadata serialization across different readers.
   - Every exported dataset includes a top-level metadata block for traceability:
     ```json
     {
       "schema_version": "1.0.0+a3f9c2",
       "export_timestamp": "2026-05-18T00:00:00Z",
       "active_layers": ["03a", "03b", "03c", "03d", "03e", "03f"],
       "pipeline_git_sha": "abc1234"
     }
     ```
   - `schema_version` follows an additive-only protocol. The minor version can be bumped, and a SHA-256 hash suffix of the sorted column names (`+<short_hash>`) is appended. This allows downstream consumers to mechanically detect schema drift between exports.
   - **No speculative columns**: because the schema is additive-only and hash-stamped, registry columns must never be pre-wired for capabilities that have not landed (e.g. gaze-telemetry columns like `any_aria_gaze` for an approach still under consideration must *not* be added until the capability ships). Whoever lands a new layer capability must update `LAYER_SUMMARY_REGISTRY` in the same change, never ahead of it — abandoned pre-wiring would permanently pollute the schema hash.

### Memory Management & Dask Fallback
To prevent Out-Of-Memory (OOM) failures when merging massive Parquets on a single node:
- **Dynamic Memory Check**: The aggregator computes an `effective_size` of the source JSONs (multiplied by a `PANDAS_INFLATION_FACTOR = 8` to account for object overhead).
- **Fallback Trigger**: If the effective size exceeds the available memory headroom (70% on hosts with ≥ 48 GB RAM, otherwise 50%), the pipeline switches from Pandas to a Dask-based chunked outer-join.
- **Partitioning & Streaming**: Dask partitions are sized dynamically. When streaming is requested (by passing `output_parquet_path`), the chunked DataFrame is written directly to a partitioned Parquet directory, bypassing full in-memory materialization.
- **Recalibration**: The `_compute_inflation_factor` helper can measure the empirical inflation ratio for a completed export, allowing operators to adjust the factor if the schema shifts toward more memory-intensive data types.

### Expected Intermediate Output
A `merged_social_features.json` (or SQLite DB) containing one record per `video_id` with all layer features as columns, ready for conversion to Parquet.

---

## Required Package Structure for Export

### 1. Rehydration Instructions (`rehydrate_dataset.py`)
To make the dataset useful to the research community, you must clearly write instructions or include a python script detailing how a downstream user can map these social features *back onto* their own valid, legally obtained copies of the source videos (Ego4D). 
- Reference exact external dataset ID numbers.
- Provide mapping schemas (e.g. "We extracted `flinch: true` at timestamp `14.5s` for Ego4D UUID `XYZ-123`").

### 2. The Result File (`social_metadata.parquet`)
Merge the final state of the layered results database into a highly compressed Apache Parquet file format. Parquets are optimized for loading into Pandas or Hugging Face `/datasets` directly. Ensure NaN/Missing columns (due to a video not triggering a specific layer, or a layer failing) are handled cleanly.

### 3. Posting to Hugging Face
If the local `result_file` review looks good, write an automated script leveraging the `huggingface_hub` Python package to initialize the dataset repository, upload the `social_metadata.parquet`, and populate a generated `README.md` Dataset Card outlining the total number of layers utilized to create the signals.

### 4. Per-Layer Datasets (`per_layer.py`)
Besides the combined master table, each layer can also be published as its own standalone Hugging Face dataset via `src/layer_04_dehydrated_export/per_layer.py` (`publish_all_layers` scans the same `03*_result.json` results directory the aggregator uses, builds one dataset per layer, and optionally creates the public repos + uploads). Each per-layer dataset carries the manifest base columns + only that layer's feature columns (with the `03x_` prefix stripped, per Aggregation Logic §3 above), plus a detailed, column-accurate interpretation card. Two behaviors are **layer-specific** and deliberately NOT applied to the combined export:
- **Unmeasured rows are dropped** (`drop_unmeasured_rows`): a video with no measured signal for *this* layer — absent, or present only as a `skipped_reason` sentinel whose sole content is an empty-JSON `[]` — is dead weight in a single-layer dataset, so it is removed, and any column left entirely empty afterward (e.g. the now-vestigial `*_skipped_reason`) is pruned. In the *combined* table that same row may still be useful (another layer scored it), so it is kept there.
- The dataset card's column table is generated from the **actual published columns** of the filtered DataFrame, so the card always matches the parquet after row/column pruning.

## Verification & Validation Check
To guarantee the export structure adheres to safety standards and schema requirements:
- **Singular Video Test**: Load the final `social_metadata.parquet` into a Pandas DataFrame inside a standalone Jupyter notebook or script. Select one random row (a single video) and attempt to cleanly follow the `rehydrate_dataset.py` instructions to rebuild the social context. If any keys are mismatched, it fails.
- **Batch Test**: Run an automated validation script across the entire merged DataFrame. Assert that there are absolutely zero raw visual bytes (no images/video buffers stored). This check explicitly scans for Python `bytes`, `numpy.ndarray` buffers, base64-encoded image strings, and raw local file path references. Validate that the row count matches exactly the number of clips successfully parsed, and perform a null-check analysis. Verify that this Pandas aggregation operates within the data limits of our **Mac Studio (M4 Max, 64 GB unified memory)** when reading multi-gigabyte Parquets into memory.

## 🧪 Resolved Issues & Implementation Refinements

1. **`LAYER_SUMMARY_REGISTRY` 03d Action Labels Never Matched — Dead Export Columns (Resolved - June 12)**:
   - **Problem**: Found in the June 12 repo audit. The registry (`src/layer_04_dehydrated_export/aggregator.py`) queried `per_person.classified_action` with `any_eq:Approach` and `any_eq:Retreat`, but Layer 03d's `_compute_proxemic_vector` emits exactly `"Approach_Intervention"`, `"Avoidance"`, or `"Neutral"` (confirmed in the June 10 50-clip output, which contains both non-neutral labels). String equality never matched, so `any_approach_detected` / `any_retreat_detected` were silently `None` on every export — filtering `any_approach_detected == True` returned zero rows even when approaches existed in the `*_raw` JSON. Same silent-degradation class the project eliminates elsewhere, here on the export surface.
   - **Solution** (Option A): Corrected the literals to `any_eq:Approach_Intervention` / `any_eq:Avoidance` and renamed the second column to `any_avoidance_detected` for accuracy (additive-only is preserved — the broken columns never materialized in a real export, so the schema hash is undamaged). Added `tests/test_layer_04.py::TestRegistryEnumSync`, which asserts every `any_eq:` literal in the registry is in the emitting layer's pinned classification vocabulary (03d + 03e), so future enum drift fails the suite instead of silently deadening a column. Centralizing enums into a shared module (Option B) was declined to preserve the "layers are independent scripts" paradigm.

## ⚠️ Unresolved Issues & Suggestions

> **Cross-reference (July 2)**: the **unified per-segment export + QA-pair emitter** for VLA curation landed as `src/layer_04_dehydrated_export/segment_dataset.py` (docs/06 Resolved #2) — `python -m layer_04_dehydrated_export.segment_dataset <results_dir>` joins 03a–03f per `(clip, task, segment, person)` with explicit null-reasons and emits `segment_rows.parquet` + confidence-gated `qa_pairs.jsonl`, alongside the existing per-layer datasets. Remaining roadmap items touching this layer: human-verified eval flags (docs/06 Issue 4) and split/richness metadata (docs/06 Issue 6), both deferred.
