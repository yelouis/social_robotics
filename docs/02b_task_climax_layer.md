...
6. **Control (negative) segments** (`SR_02B_CONTROL_SEGMENTS`, default on; docs/06 Resolved #3): flagged `is_control: true` segments **appended after** the real ones (stable `segment_index`), capped 2–3/task — `present_unreactive` (in-cluster detection farthest ≥ 10 s from the chosen climax, face-verified) and `no_audience` (midpoint of the largest ≥ 30 s bystander-free gap, no detection within 5 s of the window). Controls are measured by the 03x layers like any segment — that measurement IS the negative label. The top-level mirror selects from real segments only; `expand_task_segments` yields controls by default with `is_control`/`control_type` on each pseudo-task (`include_controls=False` to skip). `--no-controls` to disable at annotation time.
7. **Wearer pseudo-action features** (`SR_02B_WEARER_FEATURES`, default on; docs/06 Resolved #4): per segment, `wearer_hand_detections` (Node-02 wearer hand boxes re-cut to `[climax−2 s, climax+1 s]`, manifest-only) and `wearer_egomotion_proxy` (`{mean_frame_diff, peak_frame_diff, span_sec}`, downscaled 2 Hz global frame-diff, decode-gated best-effort) — the wearer's side of each (observation, pseudo-action, outcome) tuple for the latent-action VLA route.
...
> **Cross-reference (July 2)**: the VLA-curation additions to this layer both landed — **per-segment action captions** (docs/06 Resolved #1, algorithm step 5) and **control segments** (docs/06 Resolved #3, step 6) — in time for the full-991 re-annotation to produce captioned, control-balanced segments in a single pass.
