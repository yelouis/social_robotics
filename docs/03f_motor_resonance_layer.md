...
_No open issues. The camera-motion-through-bbox FP class was the last one and is now **Resolved #10** (dense-track gate, Option A). If a sparse-but-multi-detection drift class later surfaces, **Option B** (ego-motion-compensate the keypoint velocity via the existing Farneback flow) is the documented next step._
