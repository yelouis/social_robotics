...
12. **Window-Dense Boxes Lift Recall 0 → 7 — but Removing the `MIN_GENUINE_DETECTIONS` Gate Is Counterproductive (Resolved - June 29)**:
   - **Problem**: The docs/02 Issue 1 density A/B showed that feeding **window-dense** bystander boxes (`src/shared/dense_detect.py` — YOLO+ByteTrack inside the reaction windows, IoU-matched to the manifest `person_id`s) takes 03f from **0 → 7** `motor_resonance_detected` on 25 clips (recovering reactions the `MIN_GENUINE_DETECTIONS = 2` gate had been forcing to velocity 0, while *deflating* camera-motion-inflated sparse peaks like `06532156` 5.12 → 1.44). The selected remediation (the former Issue 1 → Option A) hypothesized the gate was *mooted* by dense boxes and should be replaced by per-frame **ego-motion compensation** — subtract the camera's Farneback flow from the bystander keypoint velocity, scoring only residual motion.
   - **Solution**: Implemented and A/B-validated the gate-removal + a `residual = background_flow − Δbbox` per-keypoint subtraction. It **regressed**: on the same 25 clips dense `motor_resonance` fell **7 → 6** with velocities **saturating `VELOCITY_CAP`** (for a translating bystander that residual equals `−Δbody_world`, so it *adds* the body translation back and manufactures velocity), and **sparse** rose **0 → 2** FPs (the gate's protection was lost). Root cause of the mis-design: 03f normalizes keypoints **relative to the crop/bbox** (`x / crop_diag`), so a *tracking* dense box **already cancels the camera pan** inside `(cx − px)` — an explicit flow subtraction double-compensates. The gate is therefore **not in conflict** with dense boxes: it is *trivially satisfied* by them (≥ 2 in-window detections ⇒ passes, never suppresses) while still protecting any sparse fallback — complementary, not redundant. The change was **reverted** (03f code unchanged). The full **0 → 7** recall win is delivered **entirely by feeding window-dense boxes** (docs/02 Issue 1 → Option A pre-pass, `tools/densify_manifest.py`); 03f needs **no modification** — it simply consumes the dense manifest.

## ⚠️ Unresolved Issues & Suggestions

_No open issues. The camera-motion-through-bbox FP class is **Resolved #10** (dense-track gate) and the track-explosion infeasibility is **Resolved #11** (per-clip cap). Adopting window-dense input (docs/02 Issue 1 → Option A) lifts recall **0 → 7 with no 03f change** — the gate is trivially satisfied by dense boxes and is **retained** (gate-removal was validated as counterproductive, **Resolved #12**). If a residual camera-motion class ever surfaces on *sparse-fallback* clips, ego-motion-compensating the keypoint velocity against `Δbbox` (not the raw flow) is the documented direction._
