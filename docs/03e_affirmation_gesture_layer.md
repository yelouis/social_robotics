...
15. **Median-Smoothing Saccade Suppression Activated via 32 FPS Burst on Mac Studio (Resolved - May 15)**:
    - **Problem**: Resolved Issue #10 wired a `scipy.signal.medfilt(kernel_size=3)` saccade-suppression step in `_detect_oscillation` but gated it on `fps >= MEDFILT_KERNEL * 4 * BANDPASS_HZ.high` (= 36 FPS), well above 03a's 16 FPS burst stride. The filter was dead code at every cadence 03a actually emitted, leaving the aliased saccadic energy in the 1–3 Hz passband completely unaddressed on every host — Resolved Issue #10's own analysis acknowledged this as "mathematically unavoidable at current sampling rates." On the **Mac Studio (M4 Max, 64 GB unified memory)** the 16 FPS budget was a Mac mini-era choice; the L2CS-Net inference cost at 32 FPS is now affordable, but only if the new cadence is gated on host memory so Mac mini hosts retain their budget.
    - **Solution**: Two coordinated changes. (a) In `src/layer_03a_attention/pipeline.py` added `BURST_STRIDE_SEC_HIGH_MEM = 0.03125` (32 FPS) and a per-instance `self.burst_stride_sec` set in `__init__` via `AttentionLayerPipeline.host_can_retain_resident()` — high-memory hosts (≥ 48 GB) use the 32 FPS burst; constrained hosts retain the 16 FPS `BURST_STRIDE_SEC`. The `_track_and_score` loop now references `self.burst_stride_sec`, and `processing_meta` emits `sampling_fps_burst` and `sampling_strategy` derived from the active stride (e.g. `"adaptive_8_to_32_fps"` on M4 Max). (b) In `src/layer_03e_affirmation_gesture/pipeline.py` replaced the inline `MEDFILT_KERNEL * 2 * bp_high_hz * 2` formula in `_detect_oscillation` with a new `MEDFILT_MIN_FPS = 32.0` class constant — at 32 FPS the 3-sample median window covers 94 ms, still well below the 167 ms half-period of a 3 Hz nod (Resolved Issue #10's demolish threshold), preserving correctness while engaging the filter for the first time. The original option claimed "no change to 03e itself" was sufficient; in practice the 36 FPS literal in the inline formula had to be lowered to 32 FPS for the medfilt to actually fire at the new burst cadence — the change is one constant and adds the `MEDFILT_MIN_FPS` knob to the existing class-level tuning block. New tests `TestBurstStrideMemoryGate::{test_high_memory_host_selects_32_fps_burst,test_constrained_host_retains_16_fps_burst}` in `tests/test_03a_attention.py` verify the memory gate; the existing `test_layer_03e.py` synthetic sine-wave suite continues to pass at the unchanged ~5 FPS fixture rate (medfilt remains dormant on the fixture, by design).

16. **`MAX_INTERPOLATED_FRACTION` Re-Keyed Against Upstream Gaze Model (Resolved - May 15)**:
    - **Problem**: Resolved Issue #12 hardcoded `MAX_INTERPOLATED_FRACTION = 0.3` as a class-level constant, calibrated empirically against L2CS-Net's tracking-loss distribution on Ego4D bystanders. The threshold's provenance was the gaze model 03a happens to ship today; an upstream A/B (see 03a Unresolved Issue 2 — CrossGaze migration) or any future swap of 03a's gaze backbone would silently miscalibrate 03e — a model with better face tracking would generate denser traces and make 0.3 too tight (rejecting bystanders that the new model could have produced valid traces for), while a noisier model would make 0.3 too loose (admitting traces with reconstruction levels the new model's tracking-loss profile would have classed as low quality). Nothing in the code or schema linked the 30% number to the model it was calibrated for.
    - **Solution**: Introduced an `INTERPOLATION_THRESHOLDS = {"l2cs_net_3d_gaze": 0.3, "crossgaze": 0.2, "3dgazenet": 0.25}` class-level dict and a `_detect_upstream_gaze_model(att_results)` static helper in `src/layer_03e_affirmation_gesture/pipeline.py`. `__init__` now reads `processing_meta.model_used` from the first non-`fallback_dummy` entry of `03a_attention_result.json` and, when the model is a known key, overrides the class-level `MAX_INTERPOLATED_FRACTION` with the per-model value. Unknown models, missing `processing_meta`, or runs that contain only `fallback_dummy` results all preserve the original 0.3 default, so no existing 03a output regresses. The detection logic in `process_video` continues to reference `self.MAX_INTERPOLATED_FRACTION` unchanged — the per-model override is purely an `__init__`-time assignment. New test `test_interpolation_threshold_calibrates_to_upstream_model` in `tests/test_layer_03e.py` covers all six branches (L2CS, CrossGaze, 3DGazeNet, unknown, fallback-only, missing meta).
