...
3. **Control (negative) segments (Resolved - July 2)**:
...
| 8 | **Caption describes a salient BYSTANDER, not the wearer** (July 3 E2E finding: "Bending down to pick up golf ball" was the person in frame, wearer not visibly acting) | Not machine-detectable from the caption alone; the wearer is only visible via hands in egocentric video | Acceptable noise for QA co-training (the scene description is still true); **T2/RM items whose reasoning depends on WHO acted need human verification (Issue 4)** |

Two related measurement caveats from the July 3 E2E: `no_audience` controls are clearance-checked against **Node-02 detections**, so an untracked background figure can still be visible at frame edge ("no *detected* audience" is the honest read); and bare-gerund captions ("listening") are composed with a copula by the QA renderer since the July 3 fix.
