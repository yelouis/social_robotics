...
### Issue 1: Stereo VLM gate over-fires on ordinary single-camera egocentric video
**Status**: ⚠️ Confirmed Unresolved — Originally surfaced in the May 18 100-video Ego4D E2E run (66 / 82, 80.5 %). Re-verified at scale in the May 19 **498-video** Ego4D E2E run: **325 of 402 YOLO+VLM-processed videos (80.8 %)** were rejected by the `_vlm_confirms_side_by_side_stereo` gate added in Resolved Issue #10 (`src/shared/social_presence.py:225-244`), with the rejection logged as `[SocialPresenceDetector] Stereo gate rejected <uid>.mp4: side-by-side stereoscopic capture detected`. The rejection rate is statistically indistinguishable at 4-5× the sample size, ruling out small-N noise. New at-scale evidence: spot-checking a stereo-rejected pottery-making video (`082db032-f3f2-…`, `e2e_reports/2026_05_19/frames/stereo_fp_new_2_1.jpg`, single-camera 1440×1080, `scenarios=["Crafting"]`) shows **two distinct people** with overlapping hands working on a clay vessel together — i.e. the stereo gate is now demonstrably killing genuine social positives, not just collateral solo content. The underlying Ego4D first-100 slice contains exactly **one** known true-stereo capture (`0219271c-…`, the paper-craft video from the May 17 spot-check), and that video did not even reach the stereo gate this run because it was correctly intercepted by the Resolved Issue #12 metadata-solo prefilter (`scenarios: ["Crafting"]`). Spot-checked four stereo-rejected videos at the midpoint frame:
  - `0030b1e9-c6a6-…` (`e2e_reports/2026_05_18/frames/stereo_fp_hedge_1.jpg`): single-camera 2560×1920 GoPro hedge-trimming — wearer alone, no stereo split.
  - `002ad105-bd9a-…` (`stereo_fp_hotel_2.jpg`): single-camera 1440×1080 hotel room with a TV showing YouTube — wearer alone, no stereo split (the TV grid layout appears to have confused Moondream into reporting "two halves rendered twice").
  - `000786a7-3f9d-…` (`extra_stereo_000786a7.jpg`): single-camera 2560×1920 plumbing workshop — wearer alone with two faucet handles symmetric across the frame.
  - `001e3e4e-…` (`extra_stereo_001e3e4e.jpg`): single-camera 1920×1440 close-up of a curved white surface with two ovals — wearer alone, no stereo split.
None of these have the 2:1 frame aspect or duplicated-scene geometry that defines a true stereo capture. The Moondream gate is firing on any image with two roughly-symmetric foreground regions (TV-grid panels, paired faucets, paired ovals, paired knees), not on the structural left-half=right-half duplication that Resolved Issue #10 was specified to detect. End-effect: the social-presence pass rate dropped from **3 / 100 (May 17)** to **0 / 100 (May 18)** because every single non-solo Ego4D video that previously cleared YOLO is now being killed by the stereo gate before VLM multi-person verification even runs.

**Option A (recommended)**: **Cheap geometric prefilter on aspect ratio before invoking the stereo VLM.** True side-by-side stereo captures have frame width ≈ 2× height (e.g. the May 17 false-positive `0219271c` was 2880×1080, aspect 2.67). Gate `_vlm_confirms_side_by_side_stereo` behind `img_w / img_h >= 1.9` (or pull the YOLO-pose first-pose-positive frame's `img_w`, `img_h` directly), so 4:3 / 16:9 captures never pay the VLM call and never get a false-positive. The 2880×1080 stereo class still triggers it; all 1920×1080, 1920×1440, 2560×1920, 1440×1080 captures bypass it entirely.
  - *Pros*: Zero VLM cost on the 95%+ of Ego4D that is single-camera 4:3 / 16:9 / 4:3-tall; the prefilter is one float comparison; matches the actual physical signature of side-by-side stereo rigs; restores the May 17 baseline of 3 / 100 passing (which can then be re-evaluated by the Resolved Issue #11 chin gate and the multi-person VLM).
  - *Cons*: A stereo rig that records in 16:9-per-eye stitched to 32:9 still trips the threshold (still ≥ 1.9), but a hypothetical rig that pads each eye to 4:3 and stitches to 8:3 (aspect 2.67) does too — i.e. the heuristic is correct for the realistic Ego4D rig population but assumes future stereo captures stay at ≥1.9 aspect. Falls open (no VLM check) for any ultrawide non-stereo captures, but Ego4D doesn't have those in the corpus.

**Option B**: **Sharpen the Moondream prompt with explicit negative examples and require a structural feature.** Replace the current prompt with: *"Is this a stereoscopic image where the LEFT HALF is a near-pixel-duplicate of the RIGHT HALF, as if from a stereo camera rig? You must answer YES only if you can identify the same objects rendered twice with a small horizontal parallax shift. Answer NO for ordinary photos that happen to contain symmetric or paired objects (two faucets, two TVs, two knees, grid layouts). Respond with just YES or NO."*. Optionally append a few-shot in-context example via the `messages` API.
  - *Pros*: Keeps the VLM-based detection style consistent with the other two gates (chin, multi-person); doesn't add a new code path; could catch stereo captures that don't strictly hit 2:1 aspect.
  - *Cons*: Moondream is a 1.7 GB model — its compositional reasoning ("near-pixel-duplicate with parallax shift") is weak; the current 80%+ false-positive rate suggests it is keyword-matching "two halves" rather than reasoning about structural duplication; few-shot examples cost wall-time and may not transfer; same VLM-call latency on every video that gets a pose-positive frame.

**Option C**: **Drop the stereo gate entirely and address the `0219271c`-class false positive at intake via dataset-level annotation.** Tag the handful of known stereo-rig recording locations in Ego4D's metadata (university IDs, recorder serial numbers) and reject at the manifest layer in Layer 01, bypassing the per-frame VLM round-trip.
  - *Pros*: Zero per-video runtime cost; no Moondream involvement; the stereo class is rare enough (1 / 100 in the first slice) that a static allowlist is tractable.
  - *Cons*: Requires curating a metadata-level allowlist by inspecting Ego4D's `universities` / `recording_info` fields; not all stereo captures may carry consistent identifying metadata; defers responsibility to the acquisition layer (`docs/01_dataset_acquisition.md`); a single mis-tagged stereo file still pollutes downstream layers with double-counted bystanders.

Your selection: _____

---

### Issue 2: 0 / 100 social-presence pass rate (verified at 0 / 498 on May 19)
**Status**: ⚠️ Confirmed Unresolved — Originally surfaced as 0 / 100 in the May 18 first-100 Ego4D run; re-verified at 5× scale in the May 19 498-video Ego4D E2E run (**0 / 498 passed**, 0.0 %). Of the 410 videos that ran the full multimodal pass on May 19, **325** were killed by the stereo gate (Issue 1 above), **10** failed the multi-person Moondream gate (each `0/2 confirmed across 5 VLM attempts`), and **67** never produced a single YOLO-pose head+shoulder positive. The Resolved Issue #12 metadata-solo gate skipped 88 / 498 before YOLO — its 17.7 % solo-skip rate matches the 18 / 100 from May 18, confirming the prefilter behaves as designed across the corpus. Spot-checked four of the May 19 Moondream-multi-person-rejected videos that had highly-social metadata labels:
  - `0a6d4809-352b-…` (`scenarios=["Talking with friends/housemates"]`, `e2e_reports/2026_05_19/frames/closest_pass_talking_2.jpg`): **two people walking through a doorway** are visible behind the glass partition while the wearer reads a book. Clear true positive that Moondream rejected.
  - `03634630-7d2e-…` (`scenarios=["BBQ'ing/picnics", "Walking on street"]`, `closest_pass_bbq_2.jpg`): **a second person walks ahead of the wearer in a corridor** carrying a yellow bag. Clear true positive rejected by Moondream.
  - `0049fdd8-0044-…` (`scenarios=["Grocery shopping indoors"]`, `e2e_reports/2026_05_18/frames/closest_pass_vlm1_2.jpg`, also rejected on May 18): convenience-store cashier in red shirt partially occluded by snack rack. True positive rejected.
  - `097ef48a-e26f-…` (`scenarios=["Clothes, other shopping"]`, `closest_pass_clothes_2.jpg`): wearer alone browsing the JustFab app on phone in a clothes store. **True negative** — correctly rejected.

So 3 of 4 spot-checked Moondream-rejected videos are false-negatives. Combined with the Issue #1 stereo-gate false-positives killing **at least one** demonstrably-social video (`082db032-…` pottery), the May 19 run's `0 / 498` figure is composed of: ≈ 1 unknown number of stereo-gate false-positives on real social videos + ≥ 3 known multi-person-gate false-negatives + the long tail of true-solo unscripted egocentric activity. The base yield problem from May 18 (projected ≤ 1 / 100 after Issue 1 patch) is now firmly demonstrated to be a *gate accuracy* problem rather than purely a *base-rate* problem. At ~16 s mean per processed video, the May 19 run burned 110 min for zero kept videos. Spot-checked all three Moondream-multi-person-rejected videos from May 18: `0049fdd8-…` (`closest_pass_vlm1_2.jpg`) is a convenience-store interior with a cashier *just barely visible* through merchandise (true positive that Moondream rejected); `02c40de9-…` (`closest_pass_vlm2_2.jpg`) is a deserted register counter (true negative); `002c3b5c-ed8…` (`closest_pass_vlm3_2.jpg`) is a wearer alone in a tiled kitchen (true negative).

**Option A (recommended)**: **Expand the metadata-solo prefilter to a metadata-social *positive* allowlist.** Add a complementary `_SOCIAL_SCENARIO_SUBSTRINGS = ("talking with",  "playing with", "meeting", "interviewing", "having a conversation", …)` and only run YOLO+VLM on videos whose scenarios list contains at least one positive-social keyword (or has no scenario tags at all). This is the inverse of Resolved Issue #12: instead of carving out *easy negatives*, we keep only *plausible positives*.
  - *Pros*: Directly addresses the base-rate problem; can drive the YOLO+VLM workload down to the subset that has at least a metadata-level signal of social interaction; preserves the current solo allowlist as a defensive prefilter; reuses the existing `metadata.scenarios` infrastructure.
  - *Cons*: Will drop some genuine multi-person videos whose Ego4D scenarios are silent or ambiguous (e.g. solo-tagged `"Eating"` that happens to be a family meal); the allowlist must be curated against the actual Ego4D scenario vocabulary (≈ 70 distinct strings) and revisited as the dataset grows; introduces a recall-precision trade-off that the data engineer must own.

**Option B**: **Replace Moondream with a stronger fast VLM for the multi-person gate.** The three VLM-rejected videos confirmed `0/2 across 5 attempts`, including `0049fdd8-…` (convenience store with a visible cashier) which a human spot-check accepts as a true positive. Swap the gate model from `moondream:latest` (1.7 GB) to `qwen2.5vl:3b` or `qwen2.5vl:7b` (already installed for the climax stage per Resolved Issue #7) using the same `_vlm_ask_yes_no` plumbing.
  - *Pros*: Better compositional reasoning on partially-occluded bystanders; recovers the convenience-store-cashier class; reuses an already-loaded model on ≥48 GB hosts (`Resolved Issue #2`), so no additional memory cost; reuses Resolved Issue #7's lazy tag resolution.
  - *Cons*: Per-frame latency rises from ~1 s (Moondream) to ~3 s (Qwen2.5-VL 3B) or ~7 s (7B); with up to 5 VLM attempts per pose-positive frame, the per-video budget grows ≈ 3-5× for VLM-positive videos; on the 24 GB Mac mini both YOLO and Qwen2.5-VL cannot co-reside, so the small-tier fallback (`SAF_VLM_MODEL_TIER=small`) is mandatory there.

**Option C**: **Accept the low pass rate and feed Layer 02 a broader source dataset.** The May 17 80-Charades + 20-Ego4D mixed registry hit 72% pass rate with VLM verify off (per the May 17 report executive summary); Charades-Ego is scripted multi-person content by construction. Restore Charades-Ego flow into Layer 02 once its metadata-driven task labeler is written (per Resolved Issue #6 backlog), so the filter's effective input distribution is no longer dominated by Ego4D's unscripted-solo majority.
  - *Pros*: Treats the yield problem as a *dataset-mix* problem rather than a *filter* problem, which it structurally is; unblocks Layer 03 with real positive examples while Ego4D's solo-dominant slice is treated as a long-tail source; aligns with the original four-dataset intake plan.
  - *Cons*: Doesn't actually fix Ego4D's yield — defers it; requires writing the Charades-Ego task labeler (currently blocked by Resolved Issue #6); Charades-Ego's scripted nature is its own distribution-shift risk for downstream layers trained on it.
