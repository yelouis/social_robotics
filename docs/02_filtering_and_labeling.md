...
**Option A (recommended)**: **Window-bounded densification as a pre-pass feeding the kinematic layers (03d/03f), 02 untouched.** Run `src/shared/dense_detect.py` over each clip's reaction windows to emit a densified manifest, feed that to 03d/03f, and **re-tune their sparsity-compensating gates for dense input** (see 03f Issue 1). Leave 02's corpus-wide 1/3-fps pass and the base manifest contract unchanged.
